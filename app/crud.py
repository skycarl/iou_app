...
def get_entries(db: Session, conversation_id: int):
    return db.query(models.Entry).\
        filter(models.Entry.deleted == False).\
        filter(models.Entry.conversation_id == conversation_id).all()


def get_pairs(db: Session, conversation_id: int, user1: str, user2: str):
    ...
    user1_as_sender = db.query(models.Entry.sender,
                               ...
                               models.Entry.amount).filter(
        models.Entry.deleted == False).filter(
        models.Entry.conversation_id == conversation_id).filter(
        models.Entry.sender == user1, models.Entry.recipient == user2).all()
    ...
    user2_as_sender = db.query(models.Entry.sender,
        ...
        models.Entry.deleted == False).filter(
        models.Entry.conversation_id == conversation_id).filter(
        models.Entry.sender == user2, models.Entry.recipient == user1).all()   
